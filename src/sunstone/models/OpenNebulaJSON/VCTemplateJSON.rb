...
module OpenNebulaJSON
    class VCTemplateJSON < OpenNebula::VCTemplate
        include JSONUtils

        def create(template_json)
            template_hash = parse_json(template_json, 'vctemplate')
            if OpenNebula.is_error?(template_hash)
                return template_hash
            end

            if template_hash['vctemplate_raw']
                template = template_hash['vctemplate_raw']
            else
       ...
       def perform_action(template_json)
        ...
        def instantiate(params=Hash.new)
            ...
            if params['vctemplate']
                select_network = self['VCTEMPLATE/SUNSTONE/NETWORK_SELECT']
                if (select_network && select_network.upcase == "NO")
                    params['vctemplate'].delete("NIC")
                end

                template = template_to_str(params['vctemplate'])

                ['NIC', 'SCHED_ACTION', 'SCHED_REQUIREMENTS', 'SCHED_DS_REQUIREMENTS'].each { |i|
                    if params['vctemplate'][i] && params['vctemplate'][i].empty?
                        template << "\n#{i} = []"
                    end
                }
        ...
        def clone(params=Hash.new)
            ...
            if OpenNebula.is_error?(rc)
                ...
                return VCTemplateJSON.new_with_id(rc, @client)
            end
        end
