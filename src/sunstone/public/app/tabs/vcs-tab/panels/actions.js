...
define(function(require) {
  ...
  function _html() {
    ...
    var html = '<div class="row">\
      <div class="large-12 columns">\
        <table id="scheduling_vcs_actions_table" class="info_table dataTable">\
         <thead>\
           <tr>\
              <th>' + Locale.tr("ID") + '</th>\
              ...
              <th colspan=""> </th>\
              <th><button id="add_scheduling_vcs_action" class="button small success right radius" >' + Locale.tr("Add action") + '</button></th>\
           </tr>\
          </thead>\
          <tbody id="sched_vcs_actions_body">'+
          vcsfromJSONtoActionsTable(that.element.USER_TEMPLATE.SCHED_ACTION) +
         '</tbody>\
         </table>\
        </div>\
      </div>';

    ScheduleActions.htmlTable("vcs") //only charge the resource attribute
    return html;
  }
  ...
  function _setup(context) {
    ...
    context.off('click', '#add_scheduling_vcs_action');
    context.on('click', '#add_scheduling_vcs_action', function() {
      $("#add_scheduling_vcs_action", context).attr("disabled", "disabled");
      ScheduleActions.htmlNewAction(actions, context, "vcs");
      ScheduleActions.setup(context)
      return false;
    });

    context.off("click", "#add_vcs_action_json");
    context.on("click" , "#add_vcs_action_json", function(){
      var sched_action = ScheduleActions.retrieveNewAction(context);
      if (sched_action != false) {
        $("#no_actions_tr", context).remove();
        $("#sched_vcs_actions_body").append(ScheduleActions.fromJSONtoActionsTable(sched_action));
      } else {
        return false;
      }
      ...
      Sunstone.runAction("VC.update_template", that.element.ID, template_str);

      $("#add_scheduling_vcs_action", context).removeAttr("disabled");
      return false;
    });
  ...
  function vcsfromJSONtoActionsTable(actions_array) {
    var str = ""
    var empty = '\
      <tr id="no_actions_tr">\
    ...
    $.each(actions_array, function(index, scheduling_action) {
      ...
      str += actions;
      str += vcsfromJSONtoActionRow(scheduling_action);
    });
  ...
  function vcsfromJSONtoActionRow(scheduling_action) {
