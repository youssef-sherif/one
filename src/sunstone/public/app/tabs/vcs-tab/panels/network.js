...
define(function(require) {
  ...
  function _html() {
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.attachnic")) {
      if (StateActions.enabledStateAction("VC.attachnic",
            that.element.STATE,
            that.element.LCM_STATE) &&
          OpenNebulaVC.isNICAttachSupported(that.element)) {
        html += '\
    ...
    var externalNetworkAttrs = OpenNebulaVC.retrieveExternalNetworkAttrs(that.element);
      ...
      for (var i = 0; i < nics.length; i++) {
        ...
        if (!is_pci){
          if ( (that.element.STATE == OpenNebulaVC.STATES.ACTIVE) &&
               (that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG_NIC) &&
               (nic.ATTACH == "YES") ) {
            actions = Locale.tr("attach/detach in progress")
          } else {
            if ( (Config.isTabActionEnabled("vcs-tab", "VC.detachnic")) &&
                 (StateActions.enabledStateAction("VC.detachnic", that.element.STATE, that.element.LCM_STATE))) {
              actions += '<a href="VC.detachnic" class="detachnic" ><i class="fas fa-times"/>' + Locale.tr("Detach") + '</a>'
            }
          }
        }
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.detachnic")) {
      ...
      context.on('click', '.detachnic', function() {
        ...
        Sunstone.getDialog(CONFIRM_DIALOG_ID).setParams({
          ...
          //question :
          submit : function(){
            Sunstone.runAction('VC.detachnic', that.element.ID, nic_id);
            return false;
          }
        });
