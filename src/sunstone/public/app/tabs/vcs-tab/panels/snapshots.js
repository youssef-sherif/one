...
define(function(require) {
  ...
  function _html() {
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.snapshot_create")) {
      // If VM is not RUNNING, then we forget about the attach disk form.
      if (that.element.STATE == OpenNebulaVC.STATES.ACTIVE && that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.RUNNING) {
        html += '\
    ...
    if (!snapshots.length) {
      ...
      for (var i = 0; i < snapshots.length; i++) {
        ...
        if (
           ...
           (
            that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG_SNAPSHOT)) {
          actions = Locale.tr("snapshot in progress");
        } else {
          actions = '';

          if ((that.element.STATE == OpenNebulaVC.STATES.ACTIVE &&
               that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.RUNNING)) {

            if (Config.isTabActionEnabled("vcs-tab", "VC.snapshot_revert")) {
              actions += '<a href="VC.snapshot_revert" class="snapshot_revert" ><i class="fas fa-reply"/>' + Locale.tr("Revert") + '</a> &emsp;'
            }

            if (Config.isTabActionEnabled("vcs-tab", "VC.snapshot_delete")) {
              actions += '<a href="VC.snapshot_delete" class="snapshot_delete" ><i class="fas fa-times"/>' + Locale.tr("Delete") + '</a>'
            }
          } else if (that.element.STATE == OpenNebulaVC.STATES.POWEROFF &&  that.element.HISTORY_RECORDS.HISTORY.VC_MAD == "vcenter"){
            if (Config.isTabActionEnabled("vcs-tab", "VC.snapshot_delete")) {
              actions += '<a href="VC.snapshot_delete" class="snapshot_delete" ><i class="fas fa-times"/>' + Locale.tr("Delete") + '</a>'
            }
          }
  ...
  function _setup(context) {
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.snapshot_delete")) {
      ...
      context.on('click', '.snapshot_delete', function() {
        ...
        Sunstone.runAction('VC.snapshot_delete', that.element.ID,  {"snapshot_id": snapshot_id});
        return false;
      });
    }
  }
