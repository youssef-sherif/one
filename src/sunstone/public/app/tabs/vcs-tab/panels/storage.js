...
define(function(require) {
  ...
  function _html() {
    ...
    //if (OpenNebulaVC.isNICGraphsSupported(that.element)) {
      html += '\
          <div class="row">\
  ...
  function _setup(context) {
    ...
    if (disks.length) {
      ...
      for (var i = 0; i < disks.length; i++) {
        ...
        if (
           (
            that.element.STATE == OpenNebulaVC.STATES.ACTIVE) &&
           (
            that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG_SAVEAS ||
            that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG_SAVEAS_POWEROFF ||
            that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG_SAVEAS_SUSPENDED) &&
           (
        ...
        // Attach / Detach
           ...
           (
            that.element.STATE == OpenNebulaVC.STATES.ACTIVE) &&
           (
            that.element.LCM_STATE == OpenNebulaVC.LCM_STATES.HOTPLUG) &&
           (
            disk.ATTACH = "YES")
           ) {
          ...
          if (Config.isTabActionEnabled("vcs-tab", "VC.disk_saveas")) {
            // Check if it's volatile
            if (disk.IMAGE_ID &&
                 StateActions.enabledStateAction("VC.disk_saveas", that.element.STATE, that.element.LCM_STATE)) {
              if(Array.isArray(that.element.HISTORY_RECORDS.HISTORY)){
                var historyLenght = that.element.HISTORY_RECORDS.HISTORY.length - 1;
                if(that.element.LCM_STATE != "3" || that.element.HISTORY_RECORDS.HISTORY[historyLenght].VC_MAD != "vcenter"){
                  actions += '<a href="VC.disk_saveas" class="disk_saveas nowrap" >\
              <i class="fas fa-save fa-fw" title="Saveas"></i></a> &emsp;';
                }
              } else {
                if(that.element.LCM_STATE != "3" || that.element.HISTORY_RECORDS.HISTORY.VC_MAD != "vcenter"){
                  actions += '<a href="VC.disk_saveas" class="disk_saveas nowrap" >\
              <i class="fas fa-save fa-fw" title="Saveas"></i></a> &emsp;';
                }
              }
          ...
          if (Config.isTabActionEnabled("vcs-tab", "VC.detachdisk")) {
            if (StateActions.enabledStateAction("VC.detachdisk", that.element.STATE, that.element.LCM_STATE) && !disk.CONTEXT) {
              actions += ('<a href="VC.detachdisk" class="detachdisk nowrap" >\
              <i class="fas fa-times fa-fw" title="Detach"></i></a> &emsp;');// + Locale.tr("Detach") +
              
            }
          }

          if (Config.isTabActionEnabled("vcs-tab", "VC.disk_snapshot_create")) {
            if (StateActions.enabledStateAction("VC.disk_snapshot_create", that.element.STATE, that.element.LCM_STATE) && disk.IMAGE_ID) {
              actions += ('<a href="VC.disk_snapshot_create" class="disk_snapshot_create nowrap" >\
              <i class="fas fa-camera fa-fw" title="Snapshot"></i></a> &emsp;');//+ Locale.tr("Snapshot") +
              
            }
          }
          
          if (Config.isTabActionEnabled("vcs-tab", "VC.disk_resize")) {
            if (StateActions.enabledStateAction("VC.disk_resize", that.element.STATE, that.element.LCM_STATE) && !disk.CONTEXT) {
              if(Array.isArray(that.element.HISTORY_RECORDS.HISTORY)){
                var historyLenght = that.element.HISTORY_RECORDS.HISTORY.length - 1;
                if(that.element.LCM_STATE != "3" || that.element.HISTORY_RECORDS.HISTORY[historyLenght].VC_MAD != "vcenter"){
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.attachdisk")) {
      if (!StateActions.enabledStateAction("VC.attachdisk", that.element.STATE, that.element.LCM_STATE)){
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.detachdisk")) {
      ...
      context.on('click', '.detachdisk', function() {
        ...
        Sunstone.getDialog(CONFIRM_DIALOG_ID).setParams({
          ...
          //question :
          submit : function(){
            Sunstone.runAction('VC.detachdisk', that.element.ID, disk_id);
            return false;
          }
        });
    ...
    if (Config.isTabActionEnabled("vs-tab", "VC.disk_snapshot_revert")) {
    ...
    if (Config.isTabActionEnabled("vcs-tab", "VC.disk_snapshot_delete")) {
  ...
  function _onShow(context) {
    var that = this;
    if (OpenNebulaVC.isDiskGraphsSupported(that.element)) {
      OpenNebulaVC.monitor({
        data: {
          id: that.element.ID,
          monitor: {
