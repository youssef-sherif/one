  ...
  function _setup(context) {
    ...
    $('#' + DIALOG_ID + 'Form', context).submit(function() {
      ...
      $.each(Sunstone.getDataTable(TAB_ID).elements(), function(index, elem) {
        if (that.live) {
          Sunstone.runAction("VC.migrate_live_action", elem, extra_info);
        } else {
          Sunstone.runAction("VC.migrate_action", elem, extra_info);
        }
      });
    ...
    $.each(Sunstone.getDataTable(TAB_ID).elements(), function() {
      ...
      OpenNebulaVC.show({
        ...
        success: function (request, vm_json) {
          var element = vm_json.VC;
          var hostname = OpenNebulaVC.hostnameStr(element);

          $('.confirm-resources-header', context).append(
              '<span class="radius secondary label">' +
              Locale.tr("VC") + ' ' + element.ID + ' '+ element.NAME +' ' +
