...
module OpenNebula
    class VCTemplate < PoolElement
        ...
        VC_TEMPLATE_METHODS = {
            :allocate    => "vctemplate.allocate",
            :instantiate => "vctemplate.instantiate",
            :info        => "vctemplate.info",
            :update      => "vctemplate.update",
            :delete      => "vctemplate.delete",
            :chown       => "vctemplate.chown",
            :chmod       => "vctemplate.chmod",
            :clone       => "vctemplate.clone",
            :rename      => "vctemplate.rename",
            :lock        => "vctemplate.lock",
            :unlock      => "vctemplate.unlock"
        }

        # Creates a VCTemplate description with just its identifier
        # this method should be used to create plain Template objects.
        # +id+ the id of the user
        def VCTemplate.build_xml(pe_id=nil)
            if pe_id
                obj_xml = "<VCTEMPLATE><ID>#{pe_id}</ID></VCTEMPLATE>"
            else
                obj_xml = "<VCTEMPLATE></VCTEMPLATE>"
            end

            XMLElement.build_xml(obj_xml,'VCTEMPLATE')
        end
        ...
        #######################################################################
        # XML-RPC Methods for the VCTemplate Object
        #######################################################################
        ...
        def info(extended=false)
            ...
            rc = @client.call(VC_TEMPLATE_METHODS[:info], @pe_id, extended)

            if !OpenNebula.is_error?(rc)
                initialize_xml(rc, 'VCTEMPLATE')
                rc   = nil
        ...
        def allocate(description)
            super(VC_TEMPLATE_METHODS[:allocate], description)
        end
        ...
        def delete(recursive=false)
            return call(VC_TEMPLATE_METHODS[:delete], @pe_id, recursive)
        end
        ...
        def instantiate(name="", hold=false, template="", persistent=false)
            ...
            rc = @client.call(VC_TEMPLATE_METHODS[:instantiate], @pe_id,
        ...
        def update(new_template, append=false)
            super(VC_TEMPLATE_METHODS[:update], new_template, append ? 1 : 0)
        end
        ...
        def chown(uid, gid)
            super(VC_TEMPLATE_METHODS[:chown], uid, gid)
        end
        ...
        def chmod(owner_u, owner_m, owner_a, group_u, group_m, group_a, other_u,
            ...
            return call(VC_TEMPLATE_METHODS[:chmod], @pe_id, owner_u, owner_m, owner_a, group_u,
        ...
        def clone(name, recursive=false)
            ...
            rc = @client.call(VC_TEMPLATE_METHODS[:clone], @pe_id, name, recursive)
        ...
        #   otherwise
        def rename(name)
            return call(VC_TEMPLATE_METHODS[:rename], @pe_id, name)
        end
        ...
        # Lock a Template
        def lock(level)
            return call(VC_TEMPLATE_METHODS[:lock], @pe_id, level)
        end

        # Unlock a Template
        def unlock()
            return call(VC_TEMPLATE_METHODS[:unlock], @pe_id)
        end
