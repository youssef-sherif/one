...
module OpenNebula
    class VirtualClusterPool < Pool
        ...
        VC_POOL_METHODS = {
            :info               => "vcpool.info",
            :monitoring         => "vcpool.monitoring",
            :accounting         => "vcpool.accounting",
            :showback           => "vcpool.showback",
            :calculate_showback => "vcpool.calculateshowback"
        }
        ...
        def factory(element_xml)
            OpenNebula::VirtualCluster.new(element_xml,@client)
        end
        ...
        # No arguments, returns the not-in-done VCs for the user
        # [user_id, start_id, end_id]
        # [user_id, start_id, end_id, state]
        def info(*args)
            case args.size
                when 0
                    info_filter(VC_POOL_METHODS[:info],
                                @user_id,
                                -1,
                                -1,
                                INFO_NOT_DONE)
        ...
        def info_all()
            return info_filter(VC_POOL_METHODS[:info],
                               INFO_ALL,
                               -1,
                               -1,
                               INFO_NOT_DONE)
        ...
        # Retrieves the monitoring data for all the VCs in the pool
        #
        ...
        #   OpenNebula::Error] The first level hash uses the VC ID as keys, and
        #   as value a Hash with the requested xpath expressions,
        #   and an Array of 'timestamp, value'.
        #
        # @example
        #   VC_pool.monitoring( ['MONITORING/CPU', 'MONITORING/NETTX'] )
        #
        #   {"3"=>
        #     {
        ...
        def monitoring(xpath_expressions, filter_flag=INFO_ALL)
            return super(VC_POOL_METHODS[:monitoring],
                'VC', 'LAST_POLL', xpath_expressions, filter_flag)
        end

        # Retrieves the monitoring data for all the VCs in the pool, in XML
        #
        # @param [Integer] filter_flag Optional filter flag to retrieve all or
        #   part of the Pool. Possible values: INFO_ALL, INFO_GROUP, INFO_MINE.
        #
        ...
        def monitoring_xml(filter_flag=INFO_ALL)
            return @client.call(VC_POOL_METHODS[:monitoring], filter_flag)
        end
        ...
        def calculate_showback(start_month, start_year, end_month, end_year)
            ...
            return @client.call(VC_POOL_METHODS[:calculate_showback],
                                start_month, start_year, end_month, end_year)
        end

        # Retrieves the accounting data for all the VCs in the pool
        #
        # @param [Integer] filter_flag Optional filter flag to retrieve all or
        #   part of the Pool. Possible values: INFO_ALL, INFO_GROUP, INFO_MINE
        #   or user_id
        ...
        #             "HOSTNAME"=>"dummy",
        #
        # @example :order_by_1 => VC/UID, :order_by_2 => VC/ID
        #   {"0"=>
        #       {"0"=>
        #           {"HISTORY_RECORDS"=>
        #               {"HISTORY"=> [
        ...
        # Retrieves the accounting data for all the VCs in the pool in xml
        #
        # @param [Integer] filter_flag Optional filter flag to retrieve all or
        #   part of the Pool. Possible values: INFO_ALL, INFO_GROUP, INFO_MINE
        #   or user_id
        ...
        # Retrieves the showback data for all the VCs in the pool
        #
        # @param [Integer] filter_flag Optional filter flag to retrieve all or
        #   part of the Pool. Possible values: INFO_ALL, INFO_GROUP, INFO_MINE
        #   or user_id
        ...
        # Retrieves the showback data for all the VCs in the pool, in xml
        #
        # @param [Integer] filter_flag Optional filter flag to retrieve all or
        #   part of the Pool. Possible values: INFO_ALL, INFO_GROUP, INFO_MINE
        #   or user_id
        ...
        def build_accounting(filter_flag, options, &block)
            xml_str = @client.call(VC_POOL_METHODS[:accounting],
                        filter_flag,
                        options[:start_time],
                        options[:end_time])
            ...
            xpath_array << "HISTORY[VC/GID=#{options[:group]}]" if options[:group]
        ...
        def build_showback(filter_flag, options, &block)
            xml_str = @client.call(VC_POOL_METHODS[:showback],
                        filter_flag,
                        options[:start_month],
                        options[:start_year],
                        options[:end_month],
